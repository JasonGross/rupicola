...
Module word.
  Section __.
    ...
    (* The shifts of the bedrock2 language: the amount is a word, masked to the
       bit width. *)
    Definition slu (x y: word) : word := Zmod.slu x (Zmod.unsigned y mod 2 ^ Z.log2 width).
    Definition sru (x y: word) : word := Zmod.sru x (Zmod.unsigned y mod 2 ^ Z.log2 width).
    Definition srs (x y: word) : word := Zmod.srs x (Zmod.unsigned y mod 2 ^ Z.log2 width).
    ...
    Definition ltu (x y: word) : bool := Z.ltb (Zmod.unsigned x) (Zmod.unsigned y).
    Definition lts (x y: word) : bool := Z.ltb (Zmod.signed x) (Zmod.signed y).
    ...
    Section MinMax.
      Definition minu w1 w2 := if ltu w2 w1 then w2 else w1.
      Definition mins w1 w2 := if lts w2 w1 then w2 else w1.
      Definition maxu w1 w2 := if ltu w1 w2 then w2 else w1.
      Definition maxs w1 w2 := if lts w1 w2 then w2 else w1.

      Ltac t :=
        unfold minu, maxu, mins, maxs, ltu, lts, Z.min, Z.max;
        intros;
    ...
    Lemma morph_shiftl z n:
      0 <= n < width ->
      bits.of_Z width (Z.shiftl z n) = slu (bits.of_Z width z) (bits.of_Z width n).
    Proof.
      intros; apply Zmod.unsigned_inj.
      rewrite unsigned_slu_shamtZ, !bits.unsigned_of_Z, !Z.shiftl_mul_pow2 by lia.
    ...
    Lemma morph_shiftr z n:
      0 <= n < width ->
      0 <= z < 2 ^ width ->
      bits.of_Z width (Z.shiftr z n) = sru (bits.of_Z width z) (bits.of_Z width n).
    Proof.
      intros; apply Zmod.unsigned_inj.
      rewrite unsigned_sru_shamtZ, !Z.shiftr_div_pow2 by lia.
    ...
    Lemma Z_land_ones_rotate (a: word) b (Hrange: 0 < b < width) :
      Z.land (Z.shiftl (Zmod.unsigned a) b + Z.shiftr (Zmod.unsigned a) (width - b)) (Z.ones width) =
        Zmod.unsigned (Zmod.add (slu a (bits.of_Z width b)) (sru a (Zmod.sub (bits.of_Z width width) (bits.of_Z width b)))).
    Proof.
      pose proof Zpow_facts.Zpower2_lt_lin width width_nonneg.
      rewrite <- Zmod.of_Z_sub, Z.land_ones, Zmod.unsigned_add by lia.
      rewrite unsigned_slu_shamtZ, unsigned_sru_shamtZ by lia.
      Z.push_pull_mod; reflexivity.
    Qed.
    ...
    Lemma of_Z_land_ones_rotate a b (Ha: 0 <= a < 2 ^ width) (Hb: 0 < b < width) :
      bits.of_Z width (Z.land (Z.shiftl a b + Z.shiftr a (width - b)) (Z.ones width)) =
        Zmod.add (slu (bits.of_Z width a) (bits.of_Z width b))
                 (sru (bits.of_Z width a) (Zmod.sub (bits.of_Z width width) (bits.of_Z width b))).
    Proof.
      pose proof Zpow_facts.Zpower2_lt_lin width width_nonneg.
      apply Zmod.unsigned_inj.
      rewrite <- Zmod.of_Z_sub, Zmod.unsigned_add, unsigned_slu_shamtZ, unsigned_sru_shamtZ by lia.
...
Section Byte.
  ...
  Proof.
    ...
    rewrite (byte_unsigned_land (width := width)); reflexivity.
  Qed.
  ...
  Proof.
    ...
    rewrite (byte_unsigned_xor (width := width)); reflexivity.
  Qed.
End Byte.

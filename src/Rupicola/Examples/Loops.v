...
Section Ex.
  ...
  Program Definition vect_memcpy {n1 n2} (len: word)
          ...
          (pr1: Zmod.unsigned len < Z.of_nat n1)
          (pr2: Zmod.unsigned len < Z.of_nat n2) :=
    ...
    (a1, a2).
  Next Obligation.
    pose proof bits.unsigned_range idx width_nonneg.
    pose proof bits.unsigned_range len width_nonneg.
  ...
  Program Definition vect_memcpy_s {n1 n2} (len: word)
          (a1: VectorArray.t word n1)
          (a2: VectorArray.t word n2)
          (pr1: Zmod.signed len < Z.of_nat n1)
          (pr2: Zmod.signed len < Z.of_nat n2) :=
  ...
  Instance spec_of_unsizedlist_memcpy : spec_of "unsizedlist_memcpy" :=
    fnspec! "unsizedlist_memcpy" (len: word) (a1_ptr a2_ptr : word) /
          ...
          (pr1: Zmod.unsigned len <= Z.of_nat (List.length a1))
          (pr2: Zmod.unsigned len <= Z.of_nat (List.length a2))
          R,
    { requires tr mem :=
  ...
  Program Definition incr_gallina (c: cell) : cell :=
    let/n tick := Zmod.zero in
    let/n (tick, c) :=
       ranged_for_u
         (bits.of_Z width 3) (bits.of_Z width 5)
         (fun '\< tick, c \> tok idx bounded =>
            let/n v := get c in
            let/n v := Zmod.add v idx in
            let/n c := put v in
            let/n tick := Zmod.add tick Zmod.one in
            (tok, \< tick, c \>))
         \< tick, c \> in
    (let/n v := get c in
     let/n v := Zmod.add v tick in
     let/n c := put v in
     c).
...
Module RangedForTests.
  ...
  Time Compute (ranged_for_u Zmod.zero (bits.of_Z _ 15)
                          (fun acc t idx _ =>
                             if word.ltu (bits.of_Z _ 11) idx then
                               let t' := ExitToken.break t in
                               (t', acc)
                             else
                               let acc := idx :: acc in
...
Compute sizedlist_memcpy_br2fn (width := 64).
Compute unsizedlist_memcpy_br2fn (width := 64).
Compute vect_memcpy_s_br2fn (width := 64).
